...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Contending-Machine thinks for you about the meanings.",
        epilog="Example:  contending-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | contending-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | contending-machine         #

    $ contending-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="contending-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"contending-machine: {utterance}.")
        syslog.closelog()
